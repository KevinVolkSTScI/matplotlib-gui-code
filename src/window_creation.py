...
def set_plot_layout(plotgui):
    ...
    try:
        ...
        if (nx1 < 1) | (ny1 < 1) | (nx1 > 5) | (ny1 > 5) | (n1 < 1) \
           | (n1 > nx1*ny1):
            raise ValueError
        make_plot_layout(plotgui, nx1, ny1, n1)
    except ValueError:
        tkinter.messagebox.showinfo(
            "Error",
            "There was some issue with the plot layout parameters.")

def make_plot_layout(plotgui, nx1, ny1, n1):
    """
    ...
    """
    if (nx1 != plotgui.nxplots) | (ny1 != plotgui.nyplots):
        plotgui.nyplots = ny1
        plotgui.nxplots = nx1
        ...
        for loop in range(plotgui.number_of_plots):
            try:
                value = plotgui.xparameters[loop]['xmin']
            except KeyError:
                plotgui.xparameters.append({'label': ' ',
                                            'minimum': 0.0,
                                            ...
                                            'majorgridlines': 0,
                                            'minorgridlines': 0})
                plotgui.yparameters.append({'label': ' ',
                                            'minimum': 0.0,
                                            'maximum': 1.0, 'major': 0.1,
                                            'minor': 0.1, 'logarithmic': 0,
                                            'invert': 0, 'hide': 0,
                                            ...
                                            'majorgridlines': 0,
                                            'minorgridlines': 0})
                plotgui.fontname.append('sans-serif')
                plotgui.fontsize.append('12')
                plotgui.fontweight.append('normal')
                plotgui.plot_range.append([0.0, 1.0, 0.0, 1.0])
                plotgui.original_range.append(True)
                plotgui.plot_frame.append(0.0)
                plotgui.title.append(' ')
        for loop in range(plotgui.number_of_plots):
            try:
                value = plotgui.legend_position[loop]
            except ValueError:
                plotgui.legend_position.append(None)
                plotgui.legend_labels.append(None)
                plotgui.legend_handles.append(None)
