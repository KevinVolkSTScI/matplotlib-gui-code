...
def apply_transformation(plotgui):
    ...
    try:
        ...
        if newopt == 0:
            # Note that there is no a-priori way to scale errors, since
            # the functions may include x and y both, so set these to zero.
            #
            # Of course in some instances this may be too conservative.
            ...
            plotgui.ydata[ind1]['minimum'] = numpy.min(y1) 
            plotgui.ydata[ind1]['maximum'] = numpy.max(y1) 
            plotgui.xdata[ind1]['minimum'] = numpy.min(x1) 
            plotgui.xdata[ind1]['maximum'] = numpy.max(x1) 
            plotgui.set_properties[ind1]['ymin'] = numpy.min(y1)
            plotgui.set_properties[ind1]['ymax'] = numpy.max(y1)
            plotgui.set_properties[ind1]['xmin'] = numpy.min(x1)
            plotgui.set_properties[ind1]['xmax'] = numpy.max(x1)
        else:
